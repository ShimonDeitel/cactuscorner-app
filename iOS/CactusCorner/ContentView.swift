...
struct ContentView: View {
    ...
    @ViewBuilder
    func row(for item: Plant) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(item.title)
                .font(Theme.headlineFont)
                .foregroundStyle(Theme.ink)
            Text("\(String(describing: item.species))  \u{00B7}  \(String(describing: item.wateringDays))")
                .font(.caption)
...
struct ItemEditorView: View {
    ...
    init(mode: EditorMode, onSave: @escaping (Plant) -> Void) {
        self.mode = mode
        self.onSave = onSave
        switch mode {
        case .add:
            _draft = State(initialValue: Plant(title: ""))
        case .edit(let item):
        ...
        NavigationStack {
            Form {
                Section("Plant") {
                    ...
                    TextField("Water every (days)", value: $draft.wateringDays, format: .number)
            ...
            .navigationTitle(mode == .add ? "Add Plant" : "Edit Plant")
            .toolbar {
