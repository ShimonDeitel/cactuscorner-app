import XCTest
@testable import CactusCorner

@MainActor
final class CactusCornerTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        let item = Plant(title: "Test Item")
    ...
    func testAddRespectsFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(Plant(title: "Item \(i)"), isPro: false)
        }
        let added = store.add(Plant(title: "Overflow"), isPro: false)
    ...
    func testProBypassesFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(Plant(title: "Item \(i)"), isPro: true)
        }
        let added = store.add(Plant(title: "Extra"), isPro: true)
        XCTAssertTrue(added)
        XCTAssertEqual(store.items.count, Store.freeLimit + 1)
    }

    func testDeleteByIdRemovesItem() {
        let item = Plant(title: "Delete Me")
        _ = store.add(item, isPro: false)
    ...
    func testPersistenceRoundTrip() {
        let item = Plant(title: "Persisted")
        _ = store.add(item, isPro: false)
        let reloaded = Store()
